...
-(UIImage *)exportIMG
{
    NSLog(@"testview.frame = %@" , NSStringFromCGRect(self.textView.frame));
    NSLog(@"innerview.frame = %@" , NSStringFromCGSize(self.textView.contentSize));
    CGRect frame = CGRectMake(0, 0, self.textView.contentSize.width, self.textView.contentSize.height);
    CGRect tframe = CGRectMake(frame.origin.x + self.textView.textContainerInset.left + 5, frame.origin.y + self.textView.textContainerInset.top,
                               frame.size.width - self.textView.textContainerInset.left - self.textView.textContainerInset.right - 10,
                               frame.size.height - self.textView.textContainerInset.top - self.textView.textContainerInset.bottom);
    NSLog(@"innertextview.frame = %@" , NSStringFromCGRect(tframe));
    return [self imageFromString:self.textView.attributedText viewFrame:frame textFrame:tframe];
}

- (UIImage *)imageFromString:(NSAttributedString *)string viewFrame:(CGRect)frame textFrame:(CGRect)tFrame
{
    CGSize size = CGSizeMake(frame.size.width + frame.origin.x, frame.size.height + frame.origin.y);
...
//    test.frame = CGRectMake(0, 80, CGImageGetWidth(imageRef), CGImageGetHeight(imageRef));
