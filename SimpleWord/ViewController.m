...
- (IBAction)changeSegment:(UISegmentedControl *)sender {
    ...
    if (sender.selectedSegmentIndex == 1) {
        ...
        self.htmlViewController.img = [self.wordViewController exportIMG];
        
        UIImage * img = [self.wordViewController exportIMG];
        CGImageRef imageRef = [img CGImage];
        UIImageView * test = [[UIImageView alloc] initWithImage:img];
        test.frame = CGRectMake(0, 80, 414, CGImageGetHeight(imageRef) * 414 / CGImageGetWidth(imageRef));
        //最后生成的IMG
